...
void Core::print_stats() {
  ...
  // NEW: Object sizes and histogram
  double avg_obj_size = 0.0;
  if (_total_objects > 0) {
    avg_obj_size = (double)_total_object_bytes / _total_objects;
  }
  
  spdlog::info("[Core {}] Average Core->L2 Request Size: {:.2f} bytes", _id, avg_obj_size);
  
  // Find top 2 size classes by total bytes
  uint64_t top1_bytes = 0;
  uint64_t top2_bytes = 0;
  uint64_t top1_size = 0;
  uint64_t top2_size = 0;
  
  for (auto const& [size, bytes] : _object_size_byte_counts) {
    if (bytes > top1_bytes) {
      top2_bytes = top1_bytes;
      top2_size = top1_size;
      top1_bytes = bytes;
      top1_size = size;
    } else if (bytes > top2_bytes) {
      top2_bytes = bytes;
      top2_size = size;
    }
  }
  ...
  if (_total_object_bytes > 0) {
    top1_share = (double)top1_bytes / _total_object_bytes * 100.0;
    top2_share = (double)top2_bytes / _total_object_bytes * 100.0;
  }
  
  spdlog::info("[Core {}] Object Histogram: Top-1 size {} B ({:.2f}%), Top-2 size {} B ({:.2f}%)", 
               _id, top1_size, top1_share, top2_size, top2_share);
}
...
void Core::handle_ld_inst_queue() {
  if (!_ld_inst_queue.empty()) {
    ...
    if (front->opcode == Opcode::MOVIN) {
      ...
      uint64_t obj_size = front->size * _config.dram_req_size;
      _total_object_bytes += obj_size;
      _total_objects += 1;
      _object_size_byte_counts[obj_size] += obj_size;
